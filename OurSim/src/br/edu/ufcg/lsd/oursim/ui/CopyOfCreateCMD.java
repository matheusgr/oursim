...
public class CopyOfCreateCMD {
	...
	public static void main(String[] args) throws IOException {
		...
		String resultDir = "/local/edigley/traces/oursim/17_01_2011";
		...
		int[] rodadas = ArrayBuilder.createVector(1, 3);
			...
			for (String sptFilePath : spts) {
				for (int nSites : nSitesV) {
					for (int nRes : nResV) {
						...
						String oursimPattern = java + "oursim.jar -w %s -wt %s -s %s %s -pd %s -nr %s -synthetic_av %s -o %s %s -md %s";// XXX
																																		// -erw
																																		// %s";
						...
						String shellVarDef = String.format("SPT=%s && \\\n ISD=%s && \\\n MD=%s && \\\n", inputDir + sptFilePath, inputDir + isdFilePath,
								inputDir + mdFilePath);
						cmd += NL + sep + shellVarDef + oursimTask + NC + prespotsimTask + NC + spotsimTask + NC + tearDown;
						sep = NC;
