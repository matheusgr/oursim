...
public class CreateCMDWraperTask {
	...
	public static void main(String[] args) throws IOException {
		...
		int[] nResV = new int[] { 25 };

		int[] rodadas = ArrayBuilder.createVector(2);
				...
				for (int nRes : nResV) {
					...
					for (String sptFilePath : spts) {
						...
						String shellVarDef = String.format("SPT=%s && \\\n ISD=%s && \\\n MD=%s && \\\n", inputDir + sptFilePath, inputDir + isdFilePath,
								inputDir + mdFilePath);
						// cmd += sep + shellVarDef + oursimTask + NC +
						// prespotsimTask + NC + spotsimTask + NC + tearDown;
						cmd += sep + " SPT=" + sptFilePath + NC + spotsimTask + NC + tearDown;
						sep = NC;
						spotSep = NC;
